    ...
    if (fepl.getNumber() > 1) {
      ...
      for (int i = 0; i < fepl.getNumber(); i++) {
        ...
        if (Utils.equalsApprox(current_point.getX(), next_point.getX(), TOLERANCE)) {
          ...
          slider_manager_vertical.add(new SliderBase(VERTICAL, SliderBase.PARALLEL, dir, Utils.abs(next_point.getY() - current_point.getY()), current_point));
        } else {
          if (increasing > 0) {
            if (cpx > next_point.getX()) {
              slider_manager_vertical.add(new SliderBase(VERTICAL, SliderBase.INFLECTION, true, Utils.abs(next_point.getY() - current_point.getY()), current_point));
        }
        // else {
          //  }
          }
      ...
      for (int i = 0; i < fepl.getNumber(); i++) {
        ...
        if (Utils.equalsApprox(current_point.getY(), next_point.getY(), TOLERANCE)) {
          ...
          slider_manager_horizontal.add(new SliderBase(HORIZONTAL, SliderBase.PARALLEL, dir, Utils.abs(next_point.getX() - current_point.getX()), current_point));
        } else {
          if (increasing > 0) {
            if (cpy > next_point.getY()) {
              slider_manager_horizontal.add(new SliderBase(HORIZONTAL, SliderBase.INFLECTION, true, Utils.abs(next_point.getX() - current_point.getX()), current_point));
        }
        // else {
          //  }
          }
