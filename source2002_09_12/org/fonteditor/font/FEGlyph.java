...
public class FEGlyph implements SpringConstants {
  ...
  private FEPoint drag_start = new FEPoint(0, 0);
  private FEPoint drag_end = new FEPoint(0, 0);
  ...
  public void release(Point p, GlyphDisplayOptions gdo, Selection selection) {
    ...
    if (getDragBox()) {
      FEPointList fepointlist = getFEPointList(gdo);
      FEPathList fepathlist = getFEPathList(gdo);
      int min_x = Utils.min(drag_start.getX(), drag_end.getX());
      int min_y = Utils.min(drag_start.getY(), drag_end.getY());
      int max_x = Utils.max(drag_start.getX(), drag_end.getX());
      int max_y = Utils.max(drag_start.getY(), drag_end.getY());
