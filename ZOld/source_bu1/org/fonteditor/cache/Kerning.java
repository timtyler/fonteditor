...
public class Kerning {
  ...
  public int[] getKerningOffsetsLHS() {
    if (kerning_offsets_lhs == null) {
      ImageWrapper image_wrapper = cached_glyph.getImageWrapper();
      int height = cached_glyph.getHeight();
      kerning_offsets_lhs = new int[height];
      for (int y = height; --y >= 0;) {
        kerning_offsets_lhs[y] = getLHSOffset(image_wrapper, y);
      }
  ...
  public int[] getKerningOffsetsRHS() {
    if (kerning_offsets_rhs == null) {
      ...
      for (int y = height; --y >= 0;) {
        kerning_offsets_rhs[y] = getRHSOffset(image_wrapper, y);
      }
  ...
  private int[] spreadKerningDataLHS(int[] offsets) {
    ...
    private int getOffset(ImageWrapper image, int y, int x0, int dx) {
    int[] ia = image.getArray();
  ...
  private int getRHSOffset(ImageWrapper image, int y) {
    return getOffset(image, y, cached_glyph.getWidth() - 1, -1);
  }

  private int getLHSOffset(ImageWrapper image, int y) {
