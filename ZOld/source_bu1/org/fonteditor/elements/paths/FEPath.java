...
public class FEPath implements PathConstants {
  ...
  private void ring(Graphics g, FEPoint p, Coords c) {
    g.setColor(Color.red);
    if (cfg_shape == SQUARE) {
      Rectangle r = new Rectangle(p.x - CP_SIZE, p.y - CP_SIZE, CP_SIZE << 1, CP_SIZE << 1);
  ...
  private void drawWideLine(Graphics g, Point p1, Point p2, int width, Coords c) {
    WideLine.renderRound(g, p1, p2, width, c);
  }

  private void fillCircle(Graphics g, Point p, int width, Coords c) {
    int w = width >> 1;

    g.fillOval(c.scaleX(p.x - w), c.scaleY(p.y - w), c.scaleX(width), c.scaleY(width));
  }

  private void drawCircle(Graphics g, Point p, int inner, int outer, Coords c) {
    int step = Utils.min(c.rescaleX(1), c.rescaleY(1));

    step = step >> 2;
    for (int i = inner; i < outer; i += step) {
      g.drawOval(c.scaleX(p.x - i), c.scaleY(p.y - i), c.scaleX(i << 1), c.scaleY(i << 1));
    }
  }
