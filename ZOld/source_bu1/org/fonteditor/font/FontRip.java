...
public class FontRip implements InstructionConstants {
  ...
  public static FEFont doRip(Graphics g, String font_name, boolean bold, boolean italic, int min, int max) {
    ...
    for (int i = min; i < max; i++) {
      ga.add(rip(g, "" + (char) i, font_name, font_style), i);
    }
 
    return fefont;
  }

  private static FEGlyph rip(Graphics g, String s, String font_name, int font_style) {
    ...
    //Log.log(font_name);
    int size = 256;
    Font f = new Font(font_name, font_style, size);
    GlyphVector gv = f.createGlyphVector(frc, "" + (char) number);

    shape_array[number] = gv.getOutline();
    
    // Possibly process them - extracting statistics, etc...
    // Extract the curves from the shapes...
    FEGlyph glyph = new FEGlyph(number);
  ...
  private static void reorderPaths(FEGlyph glyph) {
    ...
    glyph.makeGlyphFromInstructionStream(gdo);
